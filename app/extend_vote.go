...
func (h *VoteExtHandler) VerifyVoteExtensionHandler(ctx sdk.Context, req *abci.RequestVerifyVoteExtension) (*abci.ResponseVerifyVoteExtension, error) {
	...
	if err != nil {
		h.logger.Error("@VerifyVoteExtensionHandler: failed to unmarshal vote extension 111", "error", err)
	...
	if err != nil {
		if !errors.Is(err, collections.ErrNotFound) {
			h.logger.Error("@VerifyVoteExtensionHandler: failed to get attestation requests 222", "error", err)
			// return nil, err
		} else if len(voteExt.OracleAttestations) > 0 {
			h.logger.Error("@VerifyVoteExtensionHandler: oracle attestations length is greater than 0, should be 0 333", "voteExt", voteExt)
			// return &abci.ResponseVerifyVoteExtension{Status: abci.ResponseVerifyVoteExtension_REJECT}, nil
		} else {
			h.logger.Info("@VerifyVoteExtensionHandler: no attestation requests found, no oracle attestations to verify 444", "err", err)
		}
	} else if len(voteExt.OracleAttestations) > len(attestationRequests.Requests) {
		h.logger.Error("@VerifyVoteExtensionHandler: oracle attestations length is greater than attestation requests length 444", "voteExt", voteExt)
		// return &abci.ResponseVerifyVoteExtension{Status: abci.ResponseVerifyVoteExtension_REJECT}, nil
	}
	...
	if len(voteExt.InitialSignature.SignatureA) > 65 || len(voteExt.InitialSignature.SignatureB) > 65 {
		h.logger.Error("@VerifyVoteExtensionHandler: initial signature size is greater than 65 555", "voteExt", voteExt)
		// return &abci.ResponseVerifyVoteExtension{Status: abci.ResponseVerifyVoteExtension_REJECT}, nil
	}
	// verify the valset signature size
	if len(voteExt.ValsetSignature.Signature) > 65 {
		h.logger.Error("@VerifyVoteExtensionHandler: valset signature size is greater than 65 666", "voteExt", voteExt)
		// return &abci.ResponseVerifyVoteExtension{Status: abci.ResponseVerifyVoteExtension_REJECT}, nil
	}

	// h.logger.Info("@VerifyVoteExtensionHandler: vote extension verified", "voteExt", voteExt)
	h.logger.Info("@VerifyVoteExtensionHandler: vote extension verified 777")
